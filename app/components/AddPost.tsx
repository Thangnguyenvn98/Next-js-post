    ...
    return (
        <form className='bg-white  my-8 p-8 rounded-md' onSubmit = {submitPost}>
            <div className="flex flex-col my-4">
                <textarea onChange={(e)=>setTitle(e.target.value)}name="title" value={title} placeholder ="What's on your mind... ?" className='p-4 text-lg rounded-md my-2 bg-gray-200 text-black'></textarea>
            </div>
            <div className='flex items-center justify-between gap-2'>
                <p className={`font-bold text-sm ${title.length > 300 ? "text-red-700":"text-gray-700"}`}>{`${title.length}/300`}</p>
                <button disabled={isDisabled} className='text-sm bg-blue-400 text-white py-2 rounded-lg px-6 disabled:opacity-25' type ="submit">Create a Post</button>
            </div>
        </form>
    )
}